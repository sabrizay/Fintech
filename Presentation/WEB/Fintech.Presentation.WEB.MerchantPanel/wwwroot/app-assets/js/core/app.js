    ...
    $.ajax({
        ...
        success: function (response) {           
            if (response.success != false) {
                var newOption = new Option('Kategori Seçiniz', 0, false, false);
                $('#ProductCategory').append(newOption).trigger('change');
                $.each(response.data, function (key, value) {

                   
                    //var newOption = new Option(value.categoryFullName, value.categoryId, false, false);
           

                    $('<option/>', {
                        text: value.categoryFullName,
                        value: value.categoryId,
                        data: {
                            hasVariant: value.hasVariant
                        }
                    }).appendTo('#ProductCategory').trigger('change');
                    //$('#ProductCatagory').append(newOption).trigger('change');

                });

            }
            $('.select2').select2();
            $('#ProductCategory').select2({ placeholder: "Kategori Seçiniz" });


        },
        error: function () {
            alert("Kategoriler Alýnamadý");
        }
    });
}
        ...
        success: function (response) {
            ...
            if (response.success != false) {
                ...
                $.each(response.data, function (key, value) {
                    ...
                    $('#ProductBrand').append(newOption).trigger('change');

                });
              
                $('.select2').select2();
                $('#ProductBrand').select2({ placeholder: "Marka Seçiniz" });
            }

        },
        error: function () {
            alert("Markalar Alýnamadý");
        }
    });
}
